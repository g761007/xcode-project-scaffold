...
architecture:
  ...
  includeExample: true   # 選填；未填則依 pattern 解析，只在 pattern ∈ {mvvm, mvvm-c} 有意義
```
...
- **未填（nil）** → 依 pattern 解析：有範例的 pattern（mvvm/mvvm-c）視為 `true`、`minimal` 視為 `false`。所以選 mvvm 不必特地寫就有範例，而預設的 minimal 專案仍合法（不會踩到 §6 的 `XS1201`）。nil 於 encode 省略，未指定者的 `scaffold.yml` 不會出現這個欄位。
...
- 新增 `architecture.includeExample: Bool?`（選填，未填即 nil）。未填時依 pattern 解析（見 §3.3）；nil 於 encode 省略，故未指定者的 `scaffold.yml` 輸出不變。
- **`schemaVersion` 維持 `1`。** 新增的是選填欄位（未填即省略），向後相容；0.x 無相容承諾也不做 migration（README），沒有 bump 的理由。
...
範例開關       architecture.includeExample（scaffold.yml，Bool?；未填依 pattern 解析）

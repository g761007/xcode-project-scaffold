...
public enum UnitTestFramework: String, Codable, CaseIterable, Sendable {
    ...
    case xctest                           // v0.1 → XS0008，沒有對應模板
...
XS0008  Test framework '<name>' is not supported in this version.
```

`XS0008` 存在是因為 `testing.unit` 有三個值而模板只涵蓋兩個：`swift-testing` 有模板、`none` 表示不產生測試、`xctest` 兩者皆非。少了這條規則，選 `xctest` 的專案會拿到寫著 `import Testing` 的測試檔。
...
因此 v0.1 **沒有** `merge-yaml`、`merge-json`、`append`、`replace-generated` 這些策略。

**也沒有 per-file 的衝突策略。** 早期版本寫的是 `create-only` 與 `error` 兩種，但在 ADR-0001 的出生證明模型下 xscaffold 從不回頭改既有專案，所以每個檔案都是 create-only——真正的問題只剩「目的目錄已經有東西」，那是**目錄層級**的檢查配上 `--force`（M6）。`PlannedFile` 因此只有 `path` 與 `contents`：一個沒有消費者的契約欄位會讓讀者以為存在某種選擇。
...
├── Variants                        platform-interface，彼此不共用原始碼
│   ├── ios-uikit                   AppDelegate、SceneDelegate、RootViewController
│   └── ios-swiftui                 {{PROJECT_NAME}}App、ContentView
└── Architectures                   只提供 README 的一段散文
    └── minimal/architecture.md
```

Variant 目錄名即 `<platform>-<interface>`，直接由 schema 的 raw value 組成，不需要對照表。
...
`Resources/` 放的是**真的資產目錄**而非 `.gitkeep`：空目錄在 git 裡不存在，而 `project.yml` 的 `sources` 列了 `Resources`，目錄不存在時 `xcodegen generate` 會直接失敗。放一個最小的 `Assets.xcassets` 同時解決這件事與「新專案本來就需要 AppIcon」。

模板**編譯進二進位檔**，不隨附資源套件——理由見 [ADR-0003](../adr/0003-compile-templates-into-the-binary.md)。

### 7.3 變數代換

只有 `{{NAME}}` 代換，沒有條件式、沒有迴圈、沒有 include。任何結構性的差異都在 Swift 裡決定後以值的形式傳進模板，這樣模板讀起來就是它將要變成的檔案。

檔名也會被代換（`App/{{PROJECT_NAME}}App.swift`）。

**找不到值的佔位符是錯誤，不是原樣輸出。** 殘留的 `{{PROJECT_NAME}}` 通常仍能編譯——佔位符也出現在註解與字串字面值裡——所以它會以「生成的專案很奇怪」而非「錯誤訊息」的形式浮現。


### 7.4 Architecture overlay 在 v0.1 只產文件
...
2. `-destination 'platform=iOS Simulator,name=iPhone 16'` 有歧義——同名裝置在多個 iOS runtime 各有一台，xcodebuild 會警告並任選第一個。

   **xscaffold 自己的測試與 CI 必須指定明確的 OS 版本或 udid**，否則驗證跑在哪台機器是不確定的，這會直接牴觸可重現性主張。

   **生成專案的 `Makefile` 則刻意不釘。** 它無從得知使用者裝了哪些 runtime——`OS=latest` 會在該裝置不存在於最新 runtime 時直接失敗（實測：iPhone 16 不存在於 iOS 26.4.1），寫死 deployment target 版本則會在使用者只裝了較新 runtime 時失敗。那裡的 destination 是開發便利性而非可重現性主張的一部分，所以預設可覆寫，並在檔案裡註明如何釘住。
...
├── Resources/
│   └── Assets.xcassets/    AppIcon 與 AccentColor
├── Tests/
├── project.yml
├── scaffold.yml
├── .swiftlint.yml

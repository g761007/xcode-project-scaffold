...
/// File-scope rather than a static member purely for the linter's type-body
/// budget: the table is the point of this file and only grows.
private let validationTriggers: [ValidationTrigger] = [
    ValidationTrigger(
    ...
    ValidationTrigger(
        ...
        configuration: .validBaseline.with {
            ...
            $0.interface = .init(primary: .swiftUI)
            $0.extensions = AppExtensions(widget: .init())
        }
    ),
    ValidationTrigger(
    ...
    ValidationTrigger(
        ...
        message: "languages does not include the development language 'en'.",
        configuration: .validBaseline.with {
            $0.localization = Localization(languages: ["zh-Hant"])
        }
    ),
    ValidationTrigger(
        code: .duplicateLanguage,
        message: "Language 'en' is listed more than once.",
        configuration: .validBaseline.with {
            $0.localization = Localization(languages: ["en", "en"])
        }
    ),
    ValidationTrigger(
...
struct ValidationContractTests {
    static let triggers: [ValidationTrigger] = validationTriggers
